...
### 规格驱动开发流程
**结构化开发过程** 遵循最新的Spec-Kit方法论，结合参考项目确保质量和进度。

#### 标准SDD工作流程（严格按序执行）
1. **`/constitution`** - 建立项目治理原则和开发准则
2. **`/specify`** - 详细的功能规格文档，专注于"什么"和"为什么"
3. **`/clarify`** - 结构化澄清未充分指定的区域（最多5个针对性问题）
4. **`/plan`** - 基于参考项目经验的技术实现方案和设计决策
5. **`/tasks`** - 包含参考项目验证步骤的可执行开发任务列表
6. **`/analyze`** - 跨文档一致性和质量分析，确保实施准备就绪
7. **`/implement`** - 执行所有任务构建功能

#### 质量门禁要求
- **澄清门禁**：所有关键歧义必须在计划阶段前解决
- **一致性门禁**：跨文档分析必须通过才能开始实施
- **宪法合规性**：所有阶段都必须验证宪法原则合规性
- **参考项目验证**：每个实现都必须经过双重验证机制

#### 参考项目集成点
- **澄清阶段**：使用lua_c_analysis的技术解析澄清实现细节
- **计划阶段**：参考lua_with_cpp的架构设计和最佳实践
- **任务阶段**：包含对两个参考项目的验证任务
- **实施阶段**：持续的行为兼容性和质量标准验证
